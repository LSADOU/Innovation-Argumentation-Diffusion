...
global {
	csv_file results_csv_file <- csv_file("stochasticity/results_stochasticity_0.csv",",", string);
	...
	init {
		...
		save h to: "generated/stochasticity/result_stochasticity_intention.csv" type: text ; 
		loop r over: res_mean_intention.keys {
			...
			if (length(res_mean_intention[r]) = size_ref) {
				...
				save tos to: "generated/stochasticity/result_stochasticity_intention.csv" type: text rewrite: false; 
			}
		...
		save h to: "generated/stochasticity/result_stochasticity_polarization.csv" type: text ; 
		loop r over: res_polarization.keys {
			...
			if (length(res_polarization[r]) = size_ref) {
				...
				save tos to: "generated/stochasticity/result_stochasticity_polarization.csv" type: text rewrite: false; 
		...
		save h to: "generated/stochasticity/result_stochasticity_adoptants.csv" type: text ; 
		loop r over: res_taux_adoptants.keys {
			...
			if (length(res_taux_adoptants[r]) = size_ref) {
				...
				save tos to: "generated/stochasticity/result_stochasticity_adoptants.csv" type: text rewrite: false; 
		...
		save h to: "generated/stochasticity/result_stochasticity.csv" type: text ; 
		loop r over: res_polarization.keys {
			if (length(res_polarization[r]) = size_ref) {
				string tos <- r + "," + last(res_polarization[r])+ "," + last(res_mean_intention[r]) +"," + last(res_taux_adoptants[r]);
				save tos to: "generated/stochasticity/result_stochasticity.csv" type: text rewrite: false; 
			}
